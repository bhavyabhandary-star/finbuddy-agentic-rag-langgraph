...
| Deterministic citations, not LLM-copied ones | Wrong or fabricated sources in a compliance-relevant answer | **Found in milestone-step-6 end-to-end testing**: the prompt asked the LLM to fill in `sources` itself, and it populated the field with quoted excerpt text instead of document identifiers — a reasonable guess given the prompt never specified the format, but wrong. Fixed by dropping `sources` from what the LLM is asked to produce and building it deterministically in code from `state["retrieved_chunks"]` — citation accuracy shouldn't depend on the LLM correctly copying it, same reasoning as the `escalate_to_human` override |
